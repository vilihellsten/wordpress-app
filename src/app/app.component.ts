...
export class AppComponent {
  ...
  async seuraavaKysymys() { // napin painalluksesta html käy ngif/for avulla läpi uuden lohkon ja uudet kysymykset kehiin
    ...
    if (this.kysymysIndexi == this.Kysymykset.length - 1) { // when the last question is reached
      this.nappiTeksti = "Valmis";
      this.kyselynPiilotus = 1; // hide the survey
      await this.generateSummary(); // Call the summary generation function
    }
  ...
  async generateSummary() {// BLACK BOXIN EHDOTUS!
    const genAI = new GoogleGenerativeAI("AIzaSyDiBGfjOGyHce_PMShiZyVX7Gqou83Tnuc");
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

    const prompt = `Please summarize the following answers: ${this.vastaukset.join(", ")}`;
  ...
  async vastaus() {
    ...
    const prompt = "give me 3 first numbers of Pi";
